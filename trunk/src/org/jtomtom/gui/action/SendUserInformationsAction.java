...
public class SendUserInformationsAction extends SwingWorker<Void, Void> {
	...
	private final boolean isUserAgreeToSendInformations() {
		int answer = 
			JOptionPane.showConfirmDialog(
				    null,
				    JTomtom.theMainTranslator.getString("org.jtomtom.main.action.sendback.askforpermission"),
				    JTomtom.theMainTranslator.getString("org.jtomtom.main.action.sendback.title"),
	...
	public static final void sendInformationsNow() {
		try {
			...
			queryString.append("gps_sn=").append(URLEncoder.encode(JTomtom.getTheGPS().getDeviceSerialNumber().substring(0, 2), "UTF-8"));
			queryString.append("&chipset=").append(URLEncoder.encode(JTomtom.getTheGPS().getChipset().toString(), "UTF-8"));
			...
			HttpURLConnection conn = (HttpURLConnection) sendbackInfosURL.openConnection(JTomtom.getApplicationProxy());
			conn.setRequestProperty("USER-AGENT", JTomtom.getUserAgent() );
	...
	private static final void disableSendbackProperty() {
		try {
			JTomtom.theProperties.setUserProperty("org.jtomtom.sendbackinformations", "false");
