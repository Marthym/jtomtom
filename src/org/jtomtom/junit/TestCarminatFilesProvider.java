...
public class TestCarminatFilesProvider {
	@BeforeClass
	...
	@Test
	public void testGetTomtomInformations() {
		try {
			TomtomFilesProvider ttFilesProvider = new CarminatFilesProvider(TomtomDeviceFinder.findMountPoint());
			File ttgoBif = ttFilesProvider.getTomtomInformations();
			assertNotNull(ttgoBif);
			assertTrue(ttgoBif.exists());
		} catch (Exception e) {
